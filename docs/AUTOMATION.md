...
Run the Gemini/Antigravity heartbeat on its chosen cadence. Codex sync remains a deterministic LaunchAgent. ChatGPT live sync is owned by a daemon bootstrapped by the official ChatGPT app-server, so it must not be duplicated in a Codex automation or a second LaunchAgent. ChatGPT.app reads are serial and rate-limited.

## Automation instructions

```text
Maintain the self-hosted AI Conversation Index in the installed repository. Codex rollout collection runs independently through the deterministic `local.chat-history-index-codex-sync` LaunchAgent. ChatGPT incremental collection is bootstrapped by ChatGPT.app's own app-server and maintains one authorized App Tools socket; do not duplicate either collector in an automation heartbeat. Read skills/ai-conversation-index/SKILL.md, docs/chatgpt-app-collector.md, docs/provider-support.md, and docs/architecture.md first. Run scripts/chat-history-cli sync-gemini and scripts/chat-history-cli sync-antigravity. Never inspect ChatGPT cookies, local storage, auth headers, or private endpoints. Never advance a provider cursor if any conversation is incomplete. Never place live SQLite/WAL files in a synced folder. Finish with scripts/chat-history-cli doctor. Stay quiet on a clean no-change run; otherwise report concise imported counts and failures.
```
